...
contract Domains is ERC721URIStorage {
    // From OpenZeppelin to help keep track of tokenIds.
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    string public tld;

    // Storing NFT SVG images on chain
    string svgPartOne =
    ...
    function register(string calldata name) public payable {
        ...
        // Create the JSON metadata of NFT. We do this by combining strings and encoding as base64
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the melbs name service", "image": "data:image/svg+xml;base64,',
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 2);
        if (len == 3) {
            return 5 * 10**16; // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals). We're going with 0.05 Matic
        } else if (len == 4) {
            return 3 * 10**16; // This is 0.03
        } else {
            return 1 * 10**16; // This is 0.01
        }
    }
